...
export const googleUser = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      // Only officialElevoraAI@gmail.com should be admin
      if (user.role === "admin" && email !== "officialElevoraAI@gmail.com") {
        user.role = "user";
        await user.save();
      }
    } else {
      // ✅ CRITICAL: Always create new users with 'user' role
      // Only make admin if email is officialElevoraAI@gmail.com
      const userRole =
        email === "officialElevoraAI@gmail.com" ? "admin" : "user";
      ...
      try {
        await createNotification(
          user._id,
          "Welcome to ElevoraAI! Your account has been created successfully.",
          "success"
        );
      } catch (notifError) {
