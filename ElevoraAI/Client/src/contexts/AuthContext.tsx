...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    const savedAuth = localStorage.getItem("elevoraAiAuth");
    if (savedAuth) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem("elevoraAiAuth", JSON.stringify(state));
  }, [state]);
  ...
  const logout = () => {
    ...
    localStorage.removeItem("elevoraAiAuth");
  };
  ...
  const updateProfile = async (data: Partial<User>) => {
    ...
    try {
      ...
      const savedUsers = JSON.parse(
        localStorage.getItem("ElevoraAIUsers") || "[]"
      );
      ...
      if (userIndex !== -1) {
        ...
        localStorage.setItem("ElevoraAIUsers", JSON.stringify(savedUsers));
      }
  ...
  const deleteUser = (userId: string) => {
    ...
    localStorage.setItem("ElevoraAIUsers", JSON.stringify(filteredUsers));
